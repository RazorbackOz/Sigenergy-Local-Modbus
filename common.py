...
def generate_sigen_entity(
    ...
    #_LOGGER.debug("RBS-common.py-59-------------------------------------------------------------------------------------")
    #_LOGGER.debug("RBS-common.py-59-Generating entity, plant_name: %s, device_name: %s, entity_class: %s", plant_name, device_name, entity_class)
    #_LOGGER.debug("RBS-common.py-59-Generating entity, device_conn: %s", device_conn)
    #_LOGGER.debug("RBS-common.py-59-Generating entity, coordinator: %s", coordinator)
    ##_LOGGER.debug("RBS-common.py-59-Generating entity, entity_description: %s", entity_description)
    #_LOGGER.debug("RBS-common.py-59-Generating entity, device_type: %s", device_type)
    #_LOGGER.debug("RBS-common.py-59-Generating entity, device_info: %s", device_info)
    #_LOGGER.debug("RBS-common.py-59-Generating entity, pv_string_idx: %s", pv_string_idx)


    entities = []
    for description in entity_description:
        # _LOGGER.debug("Generating entity for description: %s", description.name)
        #_LOGGER.debug("RBS-common.py-73-Generating entity for device_type: %s, description: %s", device_type, description.name)

        # Generate PV specific entity names and IDs if applicable
        if pv_string_idx is not None:
            #_LOGGER.debug("RBS-common.py-67- Generate entities for pv_string_idx. pv_string_idx: %s", pv_string_idx)
            ...
            #_LOGGER.debug("RBS-common.py-90- Generate entities for DEVICE_TYPE_DC_CHARGER. sensor_id: %s ", sensor_id)
            #_LOGGER.debug("RBS-common.py-91- Generate entities, change device_type: %s to %s", device_type, DEVICE_TYPE_INVERTER)
            sensor_name = sensor_id
            device_type = DEVICE_TYPE_INVERTER
        else:
            sensor_name = f"{device_name} {description.name}"
            sensor_id = sensor_name
            #_LOGGER.debug("RBS-common.py-97- Generate entities for Sigenergy components. sensor_name/sensor_id: %s", sensor_name)
...
def get_source_entity_id(device_type, device_name, source_key, coordinator, hass, pv_string_idx: Optional[int] = None): # Add pv_string_idx
    ...
    try:
        ...
        _LOGGER.debug("Looking for entity with unique ID pattern: %s", unique_id_pattern)   #RBS uncommented
        ...
        # else:
            _LOGGER.debug("Found entity ID: %s for pattern %s", entity_id, unique_id_pattern)  #RBS Uncommented
