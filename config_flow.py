...
class SigenergyConfigFlow(config_entries.ConfigFlow):
    ...
    async def async_step_dhcp_plant_config(
        ...
        for i, device_id in enumerate(self._discovered_device_type[DEVICE_TYPE_INVERTER],
            ...
            _LOGGER.debug("RBS-configflow.py-352- inverter_name: %s", inverter_name)
            _LOGGER.debug("RBS-configflow.py-353- self._discovered_ip: %s", self._discovered_ip)
            _LOGGER.debug("RBS-configflow.py-354 DEFAULT_PORT: %s", DEFAULT_PORT)
            _LOGGER.debug("RBS-configflow.py-355 device_id: %s", device_id)
            _LOGGER.debug("RBS-configflow.py-356 CONF_INVERTER_HAS_DCCHARGER: %s", self._discovered_device_type[DEVICE_TYPE_DC_CHARGER])
        ...
        try:
            with _suppress_pymodbus_logging(really_suppress= False if _LOGGER.isEnabledFor(logging.DEBUG) else True):
                if not await client.connect():
                    _LOGGER.warning("RBS-configflow.py-393-Modbus connection failed to %s:%s", host, port)
                    return False

                _LOGGER.debug("RBS-configflow.py-396-Modbus connected to %s:%s. Checking device type...", host, port)
                ...
                if await self._async_try_read_register(
                    ...
                    _LOGGER.debug("RBS-configflow.py-401- not answering as a plant, client: %s, DEFAULT_PLANT_SLAVE_ID: %s, DEVICE_CHECKS[DEVICE_TYPE_PLANT]: %s", client, DEFAULT_PLANT_SLAVE_ID, DEVICE_CHECKS[DEVICE_TYPE_PLANT])
                    return False
                ...
                for device_id in range(1, 247):
                    ...
                    if not await self._async_try_read_register(
                        ...
                        _LOGGER.debug("RBS-configflow.py-409- Check if DC Charger, device_id: %s", device_id)
