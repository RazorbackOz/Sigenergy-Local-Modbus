...
class SigenergyModbusHub:
    ...
    def _validate_register_response(self, result: Any,
        ...
        if result is None or (hasattr(result, 'isError') and result.isError()):
            #RBS _LOGGER.debug("Register validation failed for address"
            #RBS              f" {register_def.address} with error: %s", result)
            _LOGGER.debug("RBS-modbus.py-201- Register validation failed for address"
                          f" {register_def.address} with error: %s  [ %s ]", result, register_def.description)
            
            return False

        registers = getattr(result, 'registers', [])
        if not registers:
            #RBS _LOGGER.debug("Register validation failed for address %s: empty response",
            #RBS              register_def.address)
            _LOGGER.debug("RBS-modbus.py-210-Register validation failed for address %s [ %s ]: empty response",
    ...
    async def _probe_single_register(
        ...
        with _suppress_pymodbus_logging(really_suppress= False if _LOGGER.isEnabledFor(logging.DEBUG) else True):
            if register.register_type == RegisterType.READ_ONLY:
                ...
                #_LOGGER.debug("RBS-modbus.py-284- _probe_single_register-READ_ONLY, address: %s, count: %s, slave: %s, result: %s", register.address, register.count, slave_id, result)
                ...
                #_LOGGER.debug("RBS-modbus.py-291- _probe_single_register-HOLDING, address: %s, count: %s, slave: %s, result: %s", register.address, register.count, slave_id, result)
            else:
                _LOGGER.debug(
            ...
            #RBS- uncommented vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
            if _LOGGER.isEnabledFor(logging.DEBUG) and not is_supported:
                _LOGGER.debug(
                    "RBS-modbus.py-303-Register %s (%s) for device %s is not supported. Result: %s, registers: %s",
        ...
        try:
            async with self._locks[key]:
                ...
                for name, register in register_defs.items():
                    _LOGGER.debug("RBS-modbus.py-327-Probing, name: %s, register: %s", name, register)
    ...
    async def async_write_register(
        ...
        _LOGGER.debug("RBS-modbud.py-467- async_write_register until data is correct *******************************")
        _LOGGER.debug("RBS-modbud.py-467- async_write_register, device_info  : %s", device_info)
        _LOGGER.debug("RBS-modbud.py-467- async_write_register, address      : %s", address)
        _LOGGER.debug("RBS-modbud.py-467- async_write_register, value        : %s", value)
        _LOGGER.debug("RBS-modbud.py-467- async_write_register, register_type: %s", register_type)
        #return  #RBS  abort actual write
    ...
    async def async_read_inverter_data(self, inverter_name: str, update_frequency:
        ...
        ### RBS - temporaly disable to prevent cluttering up log file
        _LOGGER.debug("Reading %s Inverter registers. update_frequency is %s",
                      len(registers_to_read), update_frequency)
        #_LOGGER.debug("RBS-modbus.py-873-Registers, Inverter registers_to_read: %s", registers_to_read)
        # Use the core reading logic
        return await self._async_read_device_data_core(
    ...
    async def async_write_parameter(
        ...
        _LOGGER.debug("RBS-modbus.py-1012-device_type: %s", device_type)
        _LOGGER.debug("RBS-modbus.py-1012-INVERTER_PARAMETER_REGISTERS: %s", INVERTER_PARAMETER_REGISTERS)
        _LOGGER.debug("RBS-modbus.py-1012-DC_CHARGER_PARAMETER_REGISTERS: %s", DC_CHARGER_PARAMETER_REGISTERS)
        ...
        _LOGGER.debug("RBS-modbud.py-1055- Get device_info and slave_id. device_type: %s", device_type)
        _LOGGER.debug("RBS-modbud.py-1056- Get device_info and slave_id. device_identifier: %s", device_identifier)
        _LOGGER.debug("RBS-modbud.py-1057- Get device_info and slave_id. connection_dict: %s", connection_dict)
