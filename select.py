...
def _get_grid_code_display(data, device_name): # Changed inverter_id to device_name
    ...
    try:
        ...
        #_LOGGER.debug("RBS-select.py-69-Converted grid code to int: %s", grid_code_int)

        # Look up in map
        result = GRID_CODE_MAP.get(grid_code_int)
        # _LOGGER.debug("Grid code map lookup result: %s", result)
        #_LOGGER.debug("RBS.select.py-74-Grid code (%s) map lookup result: %s", grid_code_int, result)
...
async def async_setup_entry(
    ...
    #_LOGGER.debug("RBS-select.py-179- Add inverter Selects, coordinator.hub.inverter_connections.items(): %s ", coordinator.hub.inverter_connections.items())
    for device_name, device_conn in coordinator.hub.inverter_connections.items():
        #_LOGGER.debug("RBS-select.py-181- Add inverter Selects, plant_name: %s , device_name: %s , device_conn: %s , coordinator: %s ", plant_name, device_name, device_conn, coordinator)
